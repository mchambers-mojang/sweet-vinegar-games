...
/**
 * Attaches a WebSocket signaling server to the given HTTP server.
 * Manages room creation, joining, ICE relay, and answer forwarding.
 * Returns the WebSocketServer instance.
 */
export function attachSignaling(
  httpServer: http.Server,
  options: { rooms: Map<string, Room>; roomExpiryMs: number }
): WebSocketServer {
  const { rooms, roomExpiryMs } = options;
