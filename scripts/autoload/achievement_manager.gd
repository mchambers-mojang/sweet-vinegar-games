...
func _show_toast(definition: Dictionary) -> void:
	...
	tween.tween_property(label, "scale", Vector2(1.0, 1.0), 0.2).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_BACK)
	tween.tween_property(label, "position:y", label.position.y + 18.0, 1.2).set_ease(Tween.EASE_OUT).set_trans(Tween.TRANS_SINE)
	tween.tween_property(label, "modulate:a", 0.0, 0.45).set_delay(0.8).set_ease(Tween.EASE_IN).set_trans(Tween.TRANS_QUAD)
