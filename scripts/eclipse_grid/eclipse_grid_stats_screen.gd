...
## Eclipse Grid statistics display screen
...
const SIZE_NAMES: Dictionary = {4: "Easy 4×4", 6: "Medium 6×6", 8: "Hard 8×8", 10: "Expert 10×10"}
const SIZES: Array[int] = [4, 6, 8, 10]
...
func _build_stats_ui() -> void:
	...
	_add_header("Overall")
	var total_games: int = GameStatsManager.get_counter("eclipse_grid", "games_started")
	var current_streak: int = GameStatsManager.get_counter("eclipse_grid", "current_streak")
	var best_streak: int = GameStatsManager.get_counter("eclipse_grid", "best_streak")
	...
	for s in SIZES:
		...
		var best_ms: int = GameStatsManager.get_counter("eclipse_grid", "best_s%d" % s)
		...
		var history: Array = _get_time_history(s)
		var avg: float = _avg(history)
		_add_stat_row("Average Time", TimeFormat.format_time(avg, true) if avg >= 0 else "--")

		if not history.is_empty():
			_add_time_graph(history)

		var started: int = GameStatsManager.get_counter("eclipse_grid", "started_s%d" % s)
		var completed: int = GameStatsManager.get_counter("eclipse_grid", "completed_s%d" % s)
...
func _on_reset_pressed() -> void:
	...
	dialog.confirmed.connect(func() -> void:
		GameStatsManager.clear("eclipse_grid")
		_build_stats_ui()
		dialog.queue_free()
	)
...
func _get_time_history(s: int) -> Array:
	var all: Array = GameStatsManager.get_history("eclipse_grid")
	var times: Array = []
	for entry in all:
		if entry is Dictionary and entry.get("size") == s and entry.has("time"):
			times.append(entry["time"])
	return times


func _avg(times: Array) -> float:
	if times.is_empty():
		return -1.0
	var total := 0.0
	for t in times:
