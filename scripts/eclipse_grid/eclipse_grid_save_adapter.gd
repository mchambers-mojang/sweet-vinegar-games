...
func _can_resume_from(data: Dictionary) -> bool:
	...
	# --- rejected_cells (optional; dict of cell-index strings → PLUS or MINUS ints) ---
	if not _validate_rejected_cells(data.get("rejected_cells", null), expected, "rejected_cells"):
		return false
...
## Validate that a rejected_cells dict maps string cell-index keys to PLUS or MINUS ints.
## A missing (null) entry is accepted (old saves without rejected_cells are valid).
func _validate_rejected_cells(v: Variant, max_idx: int, label: String) -> bool:
	if v == null:
	...
	for key in d.keys():
		...
		if typeof(val) != TYPE_INT \
				or (int(val) != EclipseGridSolver.PLUS and int(val) != EclipseGridSolver.MINUS):
			push_warning("EclipseGridSaveAdapter: %s value at key %s must be PLUS or MINUS" % [label, key])
			return false
	return true
