...
var rule_set: int = 0   # 0 = Standard, 1 = Anti-Knight, 2 = Anti-King, 3 = Killer, 4 = Mini
...
func _serialize_state() -> Dictionary:
	...
	# grid_spec_id is already written by logic.serialize(); also write rule_set
	return state
...
func _get_start_crash_params() -> Dictionary:
	return {"difficulty": difficulty, "rule_set": rule_set}


func _get_resume_crash_params(saved_data: Dictionary) -> Dictionary:
	return {"difficulty": saved_data.get("difficulty", 0), "rule_set": saved_data.get("rule_set", 0)}
...
func _setup_game(saved_data: Dictionary) -> void:
	...
	if is_killer and saved_data.is_empty():
		...
		board.configure_for_spec(logic.spec)
...
func _increment_stats() -> void:
	_stats.increment_counter("sudoku", "games_started")
	_stats.increment_counter("sudoku", "started_d%d" % difficulty)
...
func _on_cell_selected(index: int) -> void:
	if _is_board_locked():
		return
	_recorder.record_input(elapsed_time, "cell_selected", {"index": index})
...
func _on_number_pressed(number: int) -> void:
	if _is_board_locked():
		return
	_recorder.record_input(elapsed_time, "number_button", {
		"number": number,
		"notes_mode": notes_mode,
		"input_mode": GameRulesRegistry.get_rule("sudoku", "input_mode"),
	})
...
func _on_hint_pressed() -> void:
	if _is_board_locked() or logic.hints_used >= 1:
		return
	_crash.register_user_action("sudoku_hint_used", {"selected_index": board.selected_index})
...
func _on_erase_pressed() -> void:
	...
	if cell.is_given:
		return
	_recorder.record_input(elapsed_time, "erase_pressed", {"index": index})
...
func _on_pause_pressed() -> void:
	...
	_crash.register_user_action("sudoku_pause_toggled", {"is_paused": is_paused})


func _on_back_pressed() -> void:
	var completed: Dictionary = _recorder.finish_session("abandoned", logic.count_filled_cells(), elapsed_time, {
		"difficulty": difficulty,
		"strikes": logic.strikes,
	})
	_storage.save_replay(completed)
	_crash.register_user_action("sudoku_back_to_menu")
...
func _on_undo_pressed() -> void:
	...
	if not logic.can_undo():
		return
	_crash.register_user_action("sudoku_undo")
...
func _on_redo_pressed() -> void:
	...
	if not logic.can_redo():
		return
	_crash.register_user_action("sudoku_redo")
...
func _handle_win() -> void:
	...
	if won:
		...
		_stats.increment_counter("sudoku", "games_won")
		_stats.increment_counter("general", "current_win_streak")
		if logic.strikes == 0:
			_stats.increment_counter("sudoku", "perfect_wins")
		if elapsed_time < 300.0:
			_stats.increment_counter("sudoku", "wins_under_300s")
		if elapsed_time < 180.0:
			_stats.increment_counter("sudoku", "wins_under_180s")
	else:
		_stats.set_counter("general", "current_win_streak", 0)
	_achievements.check_stats()
...
func _play_win_celebration() -> void:
	...
	if AppTheme.is_neon:
		var center_idx := board.cells.size() / 2
		var center_rect := board.get_cell_rect(center_idx)
...
## Flash conflicting cells (e.g. knight-move conflicts) briefly in error colour.
## Cells are shown as errors for 0.6 s then reverted — the numbers are NOT changed.
func _flash_constraint_conflicts(conflict_indices: Array[int]) -> void:
	...
	tween.tween_callback(func() -> void:
		for idx in conflict_indices:
			if idx >= 0 and idx < board.cells.size():
				board.cells[idx].set_error(false)
				board.cells[idx].queue_redraw()
	)
...
func _update_number_completion() -> void:
	if logic == null:
		return
	var sym_max := logic.spec.sym_max
	for i in range(sym_max):
		if i < _number_buttons.size():
			var btn := _number_buttons[i]
			var num := i + 1
			var complete := logic.count_number_placements(num) >= sym_max
...
func _show_fail_dialog() -> void:
	...
	dialog.confirmed.connect(func() -> void:
		var completed: Dictionary = _recorder.finish_session("failed", logic.count_filled_cells(), elapsed_time, {
			"difficulty": difficulty,
			"strikes": logic.strikes,
		})
		_storage.save_replay(completed)
...
func _show_win_dialog() -> void:
	...
	var puzzle_label: String
	if rule_set == RULE_SET_MINI:
		puzzle_label = "Mini 6×6"
	else:
		puzzle_label = DIFFICULTY_NAMES[difficulty]
	dialog.dialog_text = "You solved the %s puzzle in %s!" % [puzzle_label, time_text]
...
func _restart_same_game() -> void:
	var diff := difficulty
	var rs := rule_set
	if rs == RULE_SET_KILLER or rs == RULE_SET_MINI:
...
## Rebuild number buttons to match the active grid spec (e.g. 1–6 for Mini).
## Called from _setup_game() after logic is initialised so the spec is known.
func _update_number_buttons() -> void:
	if logic == null:
		return
	var sym_max := logic.spec.sym_max
	if sym_max == 9 and _number_buttons.size() == 9:
		return  # Already correct for 9×9 — no rebuild needed.

	for child in number_container.get_children():
		child.queue_free()
	_number_buttons.clear()

	for i in range(1, sym_max + 1):
		var btn := Button.new()
		btn.text = str(i)
		btn.custom_minimum_size = Vector2(36, 50)
		btn.size_flags_horizontal = Control.SIZE_EXPAND_FILL
...
func _on_color_pressed(color: Color) -> void:
	if _is_board_locked():
		return
	_recorder.record_input(elapsed_time, "color_pressed", {
		"color": color.to_html(),
		"selected_index": board.selected_index,
	})
	var now := Time.get_ticks_msec() / 1000.0
...
func _log_game_over_analytics(won: bool) -> void:
	...
	# Leaderboard: submit for easy/medium/hard/expert (all rule sets), Evil (standard only),
	# and Mini 6×6. Modes that don't match any supported leaderboard category are skipped.
	if won:
		var mode: String
		if rule_set == RULE_SET_MINI:
			mode = "mini"
		elif difficulty <= SudokuSolver.Difficulty.EXPERT:
			...
			if rule_set == RULE_SET_ANTI_KNIGHT:
				...
				mode = diff_mode
		elif rule_set == RULE_SET_STANDARD and difficulty == SudokuSolver.Difficulty.EVIL:
			mode = DIFFICULTY_NAMES[difficulty].to_lower()
		else:
			mode = ""
		if mode != "":
...
## Returns null for standard and Mini (no extra constraints).
...
func _record_sudoku_completion(diff: int, time: float, was_strict: bool, won: bool) -> void:
	...
	if rule_set == RULE_SET_MINI:
		best_key = "best_mini"
	elif rule_set == RULE_SET_ANTI_KNIGHT:
	...
	if was_strict:
		if won:
			var streak: int = _stats.get_counter("sudoku", "current_streak") + 1
			_stats.set_counter("sudoku", "current_streak", streak)
