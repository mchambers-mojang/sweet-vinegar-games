...
func _reduce_cage_candidates(grid: Array[int], candidates: Array) -> bool:
	...
	for cage in constraint.get_cages():
		...
		for cell in empty_cells:
			possible_digits[cell] = []
		var assignment_count := _enumerate_cage_assignments(empty_cells, 0, candidates, used_digits, sum_so_far, target_sum, possible_digits)
		...
		for cell in empty_cells:
			var allowed: Array = possible_digits[cell]
...
func _enumerate_cage_assignments(empty_cells: Array[int], cell_index: int, candidates: Array, used_digits: Array[int], sum_so_far: int, target_sum: int, possible_digits: Dictionary) -> int:
	...
	for digit in current_candidates:
		...
		var sub_count := _enumerate_cage_assignments(empty_cells, cell_index + 1, candidates, next_used, next_sum, target_sum, possible_digits)
		if sub_count > 0:
			var cell_digits: Array = possible_digits[current_cell]
