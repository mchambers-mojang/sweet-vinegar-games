...
static func _are_filled_cells_valid(grid: Array[int], p_constraints: Array = []) -> bool:
	## Temporarily mutates `grid` by clearing each filled cell, validates it
	## against the remaining grid and active constraints, then restores the
	## original value before checking the next cell.
	for index in 81:
