...
## Grid specification — drives all size-dependent behaviour.
## Set this before calling init_new_game() to use a non-9×9 grid.
## Defaults to standard_9×9 for backward compatibility.
var spec: SudokuGridSpec = SudokuGridSpec.STANDARD_9X9

## Convenience accessor: total cells in the grid (derived from spec).
var grid_cells: int:
	get:
		return spec.cell_count
...
func init_new_game(diff: int, seed_value: int) -> bool:
	...
	var result: Dictionary = generator.generate(diff, seed_value, constraints, spec)
	if result.is_empty():
		return false
...
## The spec is read from the save's grid_spec_id (defaults to standard_9×9 for legacy saves).
func init_from_save(data: Dictionary) -> void:
	# Restore grid spec — legacy saves without this field default to standard_9×9
	var spec_id: String = str(data.get("grid_spec_id", "standard_9x9"))
	var saved_spec := SudokuGridSpec.from_id(spec_id)
	if saved_spec == null:
		push_warning("SudokuLogic: unknown grid_spec_id '%s' — not resumable" % spec_id)
		return
	spec = saved_spec
	...
	for key in pm_dict:
		var idx := int(key)
		if idx >= 0 and idx < spec.cell_count:
			var raw = pm_dict[key]
			if raw is Array:
...
func place_number(cell_index: int, number: int) -> PlaceResult:
	...
	if cell_index < 0 or cell_index >= spec.cell_count:
		return result
...
func use_hint_auto(preferred_index: int) -> HintResult:
	var index := -1
	if preferred_index >= 0 and preferred_index < spec.cell_count:
...
func is_cell_editable(index: int) -> bool:
	return index >= 0 and index < spec.cell_count and puzzle[index] == 0


func is_cell_correctly_placed(index: int) -> bool:
	if index < 0 or index >= spec.cell_count:
		return false
...
func _init_pencil_marks() -> void:
	pencil_marks.clear()
	pencil_marks.resize(spec.cell_count)
	for i in spec.cell_count:
		pencil_marks[i] = []
...
## Remove all pencil marks for number in the same row, column and region as placed_index.
...
func _remove_pencil_marks_for_number(placed_index: int, number: int) -> Array:
	...
	var seen: Dictionary = {}
	var n := spec.size
	var row := placed_index / n
	var col := placed_index % n
	var box_row := (row / spec.region_h) * spec.region_h
	var box_col := (col / spec.region_w) * spec.region_w

	var indices: Array[int] = []
	for i in n:
		indices.append(row * n + i)
		indices.append(i * n + col)
	for r in range(box_row, box_row + spec.region_h):
		for c in range(box_col, box_col + spec.region_w):
...
func _get_completed_units(index: int) -> Array:
	...
	if box_complete:
		...
		for r in range(box_row, box_row + spec.region_h):
			for c in range(box_col, box_col + spec.region_w):
				cells.append(r * n + c)
		var box_row_idx := box_row / spec.region_h
		var box_col_idx := box_col / spec.region_w
		var num_col_regions := spec.size / spec.region_w
		units.append({"type": "box", "unit_index": box_row_idx * num_col_regions + box_col_idx, "cells": cells})
